...
#define I2C_DEBUG printk
#define THREE_SENSOR 5
...
irqreturn_t i2c_dw_isr(int this_irq, void *dev_id)
{
	...
	if (dev->dev->power.runtime_status != RPM_ACTIVE) {
		pm_runtime_put_autosuspend(dev->dev);
		I2C_DEBUG("[I2C] (%s) runtime has a problem (%d)\n",
			__func__, dev->dev->power.runtime_status);
		return IRQ_NONE;
	}
#endif
	...
	if (!enabled || !(stat & ~DW_IC_INTR_ACTIVITY)) {
		pm_runtime_put_autosuspend(dev->dev);
		I2C_DEBUG("[I2C] (%s) stat has a problem\n", __func__);
		return IRQ_NONE;
	}
